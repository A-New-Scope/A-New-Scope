...
let fsFile = mongoose.model('fs.file', new mongoose.Schema());
...
//////////////////////////UPLOAD////////////////////////////
let isLoggedIn = function(req, res, next) { //check session active
...
app.post ('/upload', isLoggedIn, upload, function(req, res) {
  if (!req.files[0]) {
    ...
    let temp = req.files[0].originalname;
...
app.post ('/import', isLoggedIn, function(req, res) {
  fsFile.find({
    ...
    if (!data[0]) {
      ...
      let writestream = fs.createWriteStream('./src/client/imports/' + req.body.filename); //write to uploads folder
      let readstream = gfs.createReadStream({ //read from mongodb
...
passport.use('signup', new LocalStrategy(function(username, password, done) {
  process.nextTick(function() {
    User.find({'username': username}, function(err, data) {
      if (!data.length) {
        let temp = new User({ //create a new user to store in db
          username: username
        });
